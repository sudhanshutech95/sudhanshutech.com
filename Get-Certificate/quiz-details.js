document.addEventListener('DOMContentLoaded', () => {
    ...
    if (results) {
        ...
        const attemptedQuestions = results.details.filter(detail => detail.status === 'correct' || detail.status === 'incorrect');
        const attemptedQuestionsDetails = attemptedQuestions.map((detail, index) => {
            const statusClass = detail.status === 'correct' ? 'correct' :
                                detail.status === 'incorrect' ? 'incorrect' : 'skipped';
        ...
        const incorrectQuestions = results.details.filter(detail => detail.status === 'incorrect');
        const incorrectQuestionsDetails = incorrectQuestions.map((detail, index) => {
        ...
        const skippedQuestions = results.details.filter(detail => detail.status === 'skipped');
        const skippedQuestionsDetails = skippedQuestions.map((detail, index) => {
